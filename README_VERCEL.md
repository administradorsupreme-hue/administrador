...
Esta versão inclui `supabase/kaneki_store.sql`. Execute-o no SQL Editor do Supabase. Depois configure `POSTGRES_URL`, `SUPABASE_URL` e `SUPABASE_SERVICE_ROLE_KEY` nas variáveis da Vercel. O Painel do Dono usa `/api/admin/upload` para enviar imagens para o Storage `kaneki-assets`.


## Supabase — configuração pública já embutida
A `Project URL` e a `Publishable Key` do projeto Supabase estão embutidas em `index.html` e `admin.html` para o frontend. Também existe `supabase/public-config.js` como referência.

Project URL: `https://stisctpzqcemrxrwsgvo.supabase.co`

A `Publishable Key` não deve ser confundida com a `Secret Key`. A Secret Key **não** deve ser colocada no GitHub, HTML ou navegador.

## Supabase — backend
Na Vercel, configure `SUPABASE_URL` e `SUPABASE_SERVICE_ROLE_KEY` como Environment Variables. Nunca commite a service role key.
