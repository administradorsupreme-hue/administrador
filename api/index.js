...
async function chatGeneral(req,res){
  ...
  const q=await pool.query(`SELECT c.id,c.name,c.message,c.created_at AS "createdAt",u.role,u.avatar_url AS "avatarUrl" FROM chat_messages c LEFT JOIN users u ON u.id=c.user_id ORDER BY c.id DESC LIMIT 40`);
