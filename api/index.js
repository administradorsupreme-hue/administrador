...
async function oauthCallback(req,res,provider){
  const q=req.query||{};
  res.setHeader('Cache-Control','no-store');
  const stateCookie=String(req.headers.cookie||'').match(/(?:^|;\s*)oauth_state=([^;]+)/)?.[1];
  if(!q.state||q.state!==stateCookie)return fail(res,400,'Estado OAuth inválido. Tente novamente.');
