...
async function db(){
  ...
  if(!schemaPromise) schemaPromise=(async()=>{
    await pool.query(`CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY,email TEXT UNIQUE NOT NULL,name TEXT NOT NULL,phone TEXT DEFAULT '',password_hash TEXT,role TEXT NOT NULL DEFAULT 'customer',provider TEXT NOT NULL DEFAULT 'password',avatar_url TEXT DEFAULT '',created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
      ...
      CREATE TABLE IF NOT EXISTS orders (id BIGSERIAL PRIMARY KEY,user_id TEXT REFERENCES users(id) ON DELETE SET NULL,order_key TEXT UNIQUE NOT NULL,items JSONB NOT NULL,total NUMERIC NOT NULL,name TEXT,email TEXT,phone TEXT,status TEXT NOT NULL DEFAULT 'pending',delivery JSONB NOT NULL DEFAULT '[]'::jsonb,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
    ...
    await pool.query("ALTER TABLE orders ADD COLUMN IF NOT EXISTS delivery JSONB NOT NULL DEFAULT '[]'::jsonb");
    await pool.query("ALTER TABLE orders ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'pending'");
...
async function cart(req,res){
  const t=auth(req);if(!t)return fail(res,401,'Faça login para salvar o carrinho.');
  await db();const b=await body(req);const cartData=b.cart&&typeof b.cart==='object'?b.cart:{};
  await pool.query(`INSERT INTO carts(user_id,cart,updated_at) VALUES($1,$2,NOW()) ON CONFLICT(user_id) DO UPDATE SET cart=EXCLUDED.cart,updated_at=NOW()`,[t.id,JSON.stringify(cartData)]);
  const items=Object.values(cartData).filter(i=>Number(i?.qty||0)>0);
  if(!items.length)return ok(res,{ok:true,orderId:null,orderKey:b.orderKey||null});
  let orderKey=String(b.orderKey||'').trim()||('PED-'+crypto.randomUUID().replaceAll('-','').slice(0,20).toUpperCase());
  const total=items.reduce((sum,i)=>sum+(Number(i?.price)||0)*(Number(i?.qty)||0),0);
  const existing=await pool.query('SELECT id,status FROM orders WHERE order_key=$1 LIMIT 1',[orderKey]);
  const terminal=['approved','delivered','rejected','expired','cancelled','canceled'];
  if(existing.rowCount && terminal.includes(String(existing.rows[0].status||'').toLowerCase())){
    orderKey='PED-'+crypto.randomUUID().replaceAll('-','').slice(0,20).toUpperCase();
  }
  const q=await pool.query(`INSERT INTO orders(user_id,order_key,items,total,name,email,phone,status) VALUES($1,$2,$3,$4,$5,$6,$7,'cart') ON CONFLICT(order_key) DO UPDATE SET user_id=EXCLUDED.user_id,items=EXCLUDED.items,total=EXCLUDED.total,name=EXCLUDED.name,email=EXCLUDED.email,phone=EXCLUDED.phone,status='cart' RETURNING id,order_key,status`,[t.id,orderKey,JSON.stringify(items),total,String(t.name||''),String(t.email||''),String(t.phone||'')]);
  await log('cart-order',t.email);
  return ok(res,{ok:true,orderId:q.rows[0].id,orderKey:q.rows[0].order_key,status:q.rows[0].status});
}

async function orders(req,res){
  const t=auth(req);if(!t)return fail(res,401,'Faça login para registrar o pedido.');
  await db();const b=await body(req);const items=Array.isArray(b.items)?b.items:[];if(!items.length)return fail(res,400,'Carrinho vazio.');
  let orderKey=String(b.orderKey||crypto.randomUUID());
  const total=Number(b.total||0);const name=String(b.name||t.name||'');const email=String(b.email||t.email||'');const phone=String(b.phone||t.phone||'');
  const existing=await pool.query('SELECT id,status FROM orders WHERE order_key=$1 LIMIT 1',[orderKey]);
  const terminal=['approved','delivered','rejected','expired','cancelled','canceled'];
  if(existing.rowCount && terminal.includes(String(existing.rows[0].status||'').toLowerCase())) orderKey='PED-'+crypto.randomUUID().replaceAll('-','').slice(0,20).toUpperCase();
  const r=await pool.query(`INSERT INTO orders(user_id,order_key,items,total,name,email,phone,status) VALUES($1,$2,$3,$4,$5,$6,$7,'pending') ON CONFLICT(order_key) DO UPDATE SET user_id=EXCLUDED.user_id,items=EXCLUDED.items,total=EXCLUDED.total,name=EXCLUDED.name,email=EXCLUDED.email,phone=EXCLUDED.phone,status='pending' RETURNING id,order_key`,[t.id,orderKey,JSON.stringify(items),total,name,email,phone]);
  await log('order',t.email);return ok(res,{orderId:r.rows[0].id,orderKey:r.rows[0].order_key,status:'pending'});
}

async function myOrders(req,res){
  const t=auth(req);if(!t)return fail(res,401,'Faça login primeiro.');await db();
  const r=await pool.query('SELECT id,order_key,items,total,name,email,phone,status,delivery,created_at AS "createdAt" FROM orders WHERE user_id=$1 ORDER BY created_at DESC',[t.id]);
  const orders=r.rows.map(o=>{let delivery=[];try{delivery=Array.isArray(o.delivery)?o.delivery:JSON.parse(o.delivery||'[]')}catch{};return {...o,delivery:delivery.map(x=>({productId:x.productId,name:x.name,quantity:x.quantity,accounts:(x.accounts||[]).map(a=>({login:a.login,password:decryptInventorySecret(a.password_encrypted||a.password||'')}))}))};});
...
async function adminUsers(req,res){
  ...
  const t=auth(req);
  if(!t || !isOwnerEmail(t.email) || t.role!=='owner') return fail(res,403,'Somente o dono pode gerenciar administradores.');
  await db();
  const q=await pool.query('SELECT id,email,name,role,provider,created_at AS "createdAt" FROM users ORDER BY created_at DESC LIMIT 500');
  return ok(res,{users:q.rows});
}

async function adminLogs(req,res){
  const t=auth(req);
  if(!isStaffRole(t?.role)) return fail(res,403,'Acesso restrito aos administradores.');
  await db();
  res.setHeader('Cache-Control','no-store, no-cache, must-revalidate, proxy-revalidate');
  res.setHeader('Pragma','no-cache');
  const q=await pool.query('SELECT id,action,email,at FROM logs ORDER BY at DESC LIMIT 200');
  return ok(res,{logs:q.rows});
}

async function adminRoles(req,res){
  const t=auth(req); if(!t || !isOwnerEmail(t.email) || t.role!=='owner') return fail(res,403,'Somente o dono pode gerenciar administradores.');
  await db(); const b=await body(req); const userId=String(b.userId||'').trim(); const email=normalizeEmail(b.email); const action=String(b.action||'');
  if(!['grant','revoke'].includes(action)) return fail(res,400,'Ação inválida.');
  if(!userId && !email) return fail(res,400,'Informe o ID ou o Gmail do usuário.');
  const q=userId ? await pool.query('SELECT * FROM users WHERE id=$1',[userId]) : await pool.query('SELECT * FROM users WHERE LOWER(email)=LOWER($1)',[email]);
  if(!q.rowCount)return fail(res,404,'Usuário não encontrado. Cadastre essa conta no site primeiro.');
  ...
  const u=await pool.query('UPDATE users SET role=$1 WHERE id=$2 RETURNING id,email,name,role',[role,q.rows[0].id]);
...
async function adminOrders(req,res){
  ...
  res.setHeader('Cache-Control','no-store, no-cache, must-revalidate, proxy-revalidate');
  res.setHeader('Pragma','no-cache');
  const q=await pool.query('SELECT id,order_key,items,total,name,email,phone,status,delivery,created_at AS "createdAt" FROM orders ORDER BY created_at DESC LIMIT 200');
  return ok(res,{orders:q.rows});
}

async function adminOrder(req,res){
  const t=auth(req);if(!isStaffRole(t?.role))return fail(res,403,'Acesso restrito aos administradores autorizados pelo dono.');
  await db();const b=await body(req);const orderId=Number(b.orderId||0);if(!orderId)return fail(res,400,'Pedido inválido.');
  const action=String(b.status||'').toLowerCase();
  const q=await pool.query('SELECT * FROM orders WHERE id=$1',[orderId]);if(!q.rowCount)return fail(res,404,'Pedido não encontrado.');
  const order=q.rows[0];
  if(action==='expired'){
    if(order.status==='delivered')return fail(res,409,'Este pedido já foi entregue.');
    await pool.query("UPDATE orders SET status='expired' WHERE id=$1",[orderId]);
    return ok(res,{ok:true,status:'expired'});
  }
  if(action==='approved'){
    if(order.status==='delivered')return ok(res,{ok:true,status:'delivered'});
    await pool.query("UPDATE orders SET status='approved' WHERE id=$1",[orderId]);
    // Aprovação manual no painel também confirma o PIX desse pedido para o cliente.
    await pool.query("UPDATE pix_orders SET status='RECEIVED',paid_at=COALESCE(paid_at,NOW()) WHERE order_id=$1 AND status IN ('PENDING','RECEIVED')",[orderId]).catch(()=>{});
    return ok(res,{ok:true,status:'approved'});
  }
  if(action==='rejected'){
    if(order.status==='delivered')return fail(res,409,'Este pedido já foi entregue.');
  ...
  if(action!=='delivered')return fail(res,400,'Status de pedido inválido.');
  if(order.status==='delivered')return ok(res,{ok:true,status:'delivered',delivery:order.delivery||[]});

  const client=await pool.connect();
  try{
    await client.query('BEGIN');
    const locked=await client.query('SELECT * FROM orders WHERE id=$1 FOR UPDATE',[orderId]);
    if(!locked.rowCount)throw Object.assign(new Error('Pedido não encontrado.'),{status:404});
    const current=locked.rows[0];
    if(current.status==='delivered'){await client.query('COMMIT');return ok(res,{ok:true,status:'delivered',delivery:current.delivery||[]});}
    ...
    for(const item of items){
      ...
      const product=await client.query('SELECT product_id,name,stock FROM products WHERE product_id=$1',[productId]);
      if(!product.rowCount)continue;
      const invCount=await client.query('SELECT COUNT(*)::int AS count FROM inventory_accounts WHERE product_id=$1',[productId]);
      const customized=Number(invCount.rows[0].count)>0;
      const accounts=[];
      if(customized){
        const rows=await client.query('SELECT id,login,password_encrypted FROM inventory_accounts WHERE product_id=$1 ORDER BY created_at ASC,id ASC FOR UPDATE SKIP LOCKED LIMIT $2',[productId,qty]);
        if(rows.rows.length<qty)throw Object.assign(new Error(`Estoque personalizado insuficiente para ${product.rows[0].name||productId}. Disponíveis: ${rows.rows.length}; necessários: ${qty}.`),{status:409});
        for(const row of rows.rows){accounts.push({login:row.login,password_encrypted:row.password_encrypted});await client.query('DELETE FROM inventory_accounts WHERE id=$1',[row.id]);}
        const left=await client.query('SELECT COUNT(*)::int AS count FROM inventory_accounts WHERE product_id=$1',[productId]);
        await client.query('UPDATE products SET stock=$1,updated_at=NOW() WHERE product_id=$2',[String(left.rows[0].count),productId]);
      }
      delivery.push({productId,name:product.rows[0].name||item.name||productId,quantity:qty,accounts});
    }
    await client.query("UPDATE orders SET status='delivered',delivery=$2::jsonb WHERE id=$1",[orderId,JSON.stringify(delivery)]);
    await client.query("UPDATE pix_orders SET status='RECEIVED',paid_at=COALESCE(paid_at,NOW()) WHERE order_id=$1 AND status IN ('PENDING','RECEIVED')",[orderId]);
    await client.query('COMMIT');
    await log('order-delivered',current.email||'');
    return ok(res,{ok:true,status:'delivered',delivery});
  }catch(e){try{await client.query('ROLLBACK')}catch{};return fail(res,e.status||500,e.message||'Não foi possível entregar o pedido.');}
...
async function adminSummary(req,res){
  ...
  const [u,o,tic,p,l,ia]=await Promise.all([
    ...
    pool.query('SELECT id,order_key,items,total,name,email,phone,status,created_at AS "createdAt" FROM orders ORDER BY created_at DESC LIMIT 200'),
...
module.exports = async (req,res) => {
 try{
  ...
  if(route==='admin/users' && req.method==='GET') return adminUsers(req,res);
  if(route==='admin/logs' && req.method==='GET') return adminLogs(req,res);
  if(route==='admin/orders' && req.method==='GET') return adminOrders(req,res);
  if(route==='admin/roles' && req.method==='POST') return adminRoles(req,res);
  if(route==='admin/order' && req.method==='POST') return adminOrder(req,res);
