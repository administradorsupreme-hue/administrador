...
async function chatGeneral(req,res){
  ...
  const q=await pool.query(`SELECT c.id,c.name,c.message,c.created_at AS "createdAt",u.role,u.avatar_url AS "avatarUrl" FROM chat_messages c LEFT JOIN users u ON u.id=c.user_id ORDER BY c.id DESC LIMIT 80`);
...
async function adminSettings(req,res){
  try{
    const t=auth(req); if(!t || !isOwnerEmail(t.email) || t.role!=='owner' || t.provider!=='google') return fail(res,403,'Acesso restrito: entre com Google usando administradorsupreme@gmail.com.');
    await db();
...
async function adminUpload(req,res){
  const t=auth(req);
  if(!t || !isOwnerEmail(t.email) || t.role!=='owner' || t.provider!=='google') return fail(res,403,'Acesso restrito: somente o dono pode enviar arquivos.');
