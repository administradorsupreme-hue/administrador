...
function signOAuthState(provider){
  const secret=process.env.SESSION_SECRET;
  if(!secret) throw new Error('SESSION_SECRET não configurado na Vercel.');
  const payload=Buffer.from(JSON.stringify({provider,iat:Date.now(),nonce:crypto.randomBytes(24).toString('hex')})).toString('base64url');
  const sig=crypto.createHmac('sha256',secret).update(payload).digest('base64url');
  return `${payload}.${sig}`;
}
function verifyOAuthState(state,provider){
  try{
    if(!state || !process.env.SESSION_SECRET) return false;
    ...
    return p.provider===provider && Number(p.iat)>Date.now()-10*60*1000 && Number(p.iat)<=Date.now()+60*1000;
  }catch{return false;}
}
...
async function adminSettings(req,res){
  const t=auth(req); if(!t || !isOwnerEmail(t.email) || t.role!=='owner' || t.provider!=='google') return fail(res,403,'Acesso restrito: entre com Google usando administradorsupreme@gmail.com.'); await db();
  const b=await body(req); const allowed=['siteName','logoUrl','backgroundImageUrl','backgroundOpacity','heroBackgroundUrl','heroMediaUrl','heroMediaType','heroBannerEnabled','heroBannerUrl','heroBannerType','heroBannerHeight','heroBannerPosition','heroBannerObjectFit','heroBannerAutoplay','heroBannerLoop','heroBannerMuted','heroBannerOverlay','heroBannerLink','customCss','pixFee','whatsappNumber','heroTitle','heroAccent','heroDescription','heroButton','heroBadge1','heroBadge2','tutorialTitle','tutorialSubtitle','tutorialVideoUrl','discordUrl','supportUrl','whatsappUrl','footerText','categories_json','primaryColor','primaryDark','backgroundColor','surfaceColor','cardColor','textColor','mutedColor','accentText',...Object.keys(b||{}).filter(k=>k.startsWith('content_')||k.startsWith('visible_')||k.startsWith('link_')||k.startsWith('image_')||k.startsWith('button_'))];
...
async function oauthStart(req,res,provider){
  ...
  // The OAuth state is signed with SESSION_SECRET, so the callback does not depend on a browser cookie.
...
async function oauthCallback(req,res,provider){
  const q=req.query||{};
  res.setHeader('Cache-Control','no-store');
  if(!verifyOAuthState(String(q.state||''),provider))return fail(res,400,'Estado OAuth inválido. Tente novamente.');
  ...
  res.setHeader('Set-Cookie',`kaneki_session=${token}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=2592000`);
  await log(`oauth_${provider}`,email);
  return res.writeHead(302,{Location:`${baseUrl(req)}/?login=success`}).end();
}


const PIX_KEY = 'administradorsupreme@gmail.com';
...
function buildPixPayload(amount, orderKey=''){
...
async function createPix(req,res){
  try{
    ...
    const payload=buildPixPayload(amount,orderKey);
    ...
    return ok(res,{ok:true,orderId,orderKey,qrCodeId:orderKey,amount,fee,subtotal,payload,qrImage:'',expiresAt,pixKey:PIX_KEY});
  }catch(e){
